        ...
        namespace manipulators
        {
            namespace unary
            {
                namespace acc
                {
                    ...
                    struct Drift2DifferentDir : private T_ValueFunctor
                    {
                        ...
                        HDINLINE void operator()(T_Particle& particle)
                        {
                            ...
                            // even particles get positive drift, odd particles negative drift
                            uint32_t const parId = particle[particleId_];
                            int32_t const negate = ((parId%126)<126/2 )? 1 : -1;
                            // printf("%d", negate);
                            float3_X const normDir = negate * driftDirection / pmacc::math::l2norm(driftDirection);
                     ...
                     */
                    template<typename T_ParamClass, typename T_ValueFunctor>
                    struct DriftWithFlag : private T_ValueFunctor
                    {
                        /** manipulate the speed of the particle
                         *
                         * @tparam T_Particle pmacc::Particle, particle type
                        ...
                        HDINLINE void operator()(T_Particle& particle)
                        {
                            using ParamClass = T_ParamClass;
                            using ValueFunctor = T_ValueFunctor;
                            // if(particle[driftFlag_] == true ) return; // only apply if flag is not set
                            // particle[driftFlag_] = true; // set the flag since we applied the drift
